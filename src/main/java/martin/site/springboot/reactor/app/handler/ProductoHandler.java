...
public class ProductoHandler {
	...
	public Mono<ServerResponse> crear(ServerRequest request) {
		...
		return producto.flatMap(p -> {
			...
			if(errors.hasErrors()) {
				...
				return service.save(p).flatMap(pbd -> ServerResponse.created(URI.create("api/productos/v2/".concat(pbd.getId())))
						.contentType(MediaType.APPLICATION_JSON)
						.bodyValue(pbd));
			}
		});
	...
	public Mono<ServerResponse> editar(ServerRequest request) {
		...
		return productoDB.zipWith(producto, (db, req) -> {
			...
			return db;
		}).flatMap(p -> ServerResponse.created(URI.create("api/productos/v2/".concat(p.getId())))
				.contentType(MediaType.APPLICATION_JSON)
				.bodyValue(p))
				.switchIfEmpty(ServerResponse.notFound().build());
	}
